	...
	LPWSAOVERLAPPED lpOverlapped, DWORD dwFlags)
{
	...
	retval = WSAGetOverlappedResult(ptr->sock, overlapped,
		&custTransferred, FALSE, &flags);
	if (retval == FALSE || custTransferred == 0)
	{
	...
	case LOGIN_STATE:
	case MENU_SELECT_STATE:
		result = CompleteRecv(ptr, custTransferred);
		switch (result)
		{
		case SOC_ERROR:
			return;
		...
		break;
	case RESULT_SEND_STATE:
		result = CompleteSend(ptr, custTransferred);
		switch (result)
		{
		case SOC_ERROR:
			return;
