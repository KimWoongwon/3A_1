...
struct CUSTOMVERTEX
{
    ...
    CUSTOMVERTEX(float x, float y, float z, D3DCOLOR _color, float _tu, float _tv)
    {
...
HRESULT InitGeometry()
{
    ...
    if( FAILED( D3DXCreateTextureFromFile( g_pd3dDevice, L"images.bmp", &g_pTexture ) ) )
    {
        // If texture is not in current folder, try parent folder
        if( FAILED( D3DXCreateTextureFromFile( g_pd3dDevice, L"..\\images.bmp", &g_pTexture ) ) )
        {
            MessageBox( NULL, L"Could not find images.bmp", L"Textures.exe", MB_OK );
            return E_FAIL;
        }
    }
    ...
    pVertices[0] = CUSTOMVERTEX(0.0f, 1.0f, 0.0f, 0xffffffff, 0.0f, 0.0f);
    pVertices[1] = CUSTOMVERTEX(0.0f, 0.0f, 0.0f, 0xffffffff, 0.0f, 1.0f);
    pVertices[2] = CUSTOMVERTEX(-1.0f, 0.0f, 0.0f, 0xffffffff, 1.0f, 1.0f);

    pVertices[3] = CUSTOMVERTEX(0.0f, 1.0f, 0.0f, 0xffffffff, 0.0f, 0.0f);
    pVertices[4] = CUSTOMVERTEX(-1.0f, 1.0f, 0.0f, 0xffffffff, 1.0f, 0.0f);
    pVertices[5] = CUSTOMVERTEX(-1.0f, 0.0f, 0.0f, 0xffffffff, 1.0f, 1.0f);
...
VOID SetupMatrices()
{
    ...
    D3DXVECTOR3 vLookatPt( -0.5f, 0.0f, 0.0f );
