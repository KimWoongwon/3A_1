...
HRESULT InitGeometry()
{
    ...
    if (FAILED(D3DXCreateTextureFromFile(g_pd3dDevice, L"Image.bmp", &g_pTexture2)))
    {
        // If texture is not in current folder, try parent folder
        if (FAILED(D3DXCreateTextureFromFile(g_pd3dDevice, L"..\\Image.bmp", &g_pTexture2)))
        {
            MessageBox(NULL, L"Could not find Image.bmp", L"Textures.exe", MB_OK);
            return E_FAIL;
        }
    }

#pragma region g_pVB_01_Setting
    ...
    g_pVB->Unlock();
#pragma endregion
#pragma region g_pVB_02_Setting
    // Create the vertex buffer.
    if (FAILED(g_pd3dDevice->CreateVertexBuffer(50 * 2 * sizeof(CUSTOMVERTEX),
        0, D3DFVF_CUSTOMVERTEX,
