...
HRESULT InitGeometry()
{
    ...
    for (DWORD i = 0; i < 50; i++)
    {
        ...
        g_Vertices[2 * i + 0].position = D3DXVECTOR3(sinf(theta) - 1, -1.0f, cosf(theta));
        g_Vertices[2 * i + 0].normal = D3DXVECTOR3(sinf(theta) - 1, 0.0f, cosf(theta));
        g_Vertices[2 * i + 1].position = D3DXVECTOR3(sinf(theta) - 1, 1.0f, cosf(theta));
        g_Vertices[2 * i + 1].normal = D3DXVECTOR3(sinf(theta) - 1, 0.0f, cosf(theta));
    }
    ...
    VOID* gpVertices;
    if (FAILED(g_pVB->Lock(0, sizeof(g_Vertices), (void**)&gpVertices, 0)))
        return E_FAIL;
    memcpy(gpVertices, g_Vertices, sizeof(g_Vertices));
    g_pVB->Unlock();
    ...
    //{
    //    FLOAT theta = ( 2 * D3DX_PI * i ) / ( 50 - 1 );
    //    pVertices[2 * i + 0].position = D3DXVECTOR3( sinf( theta )-1, -1.0f, cosf( theta ) );
    //    pVertices[2 * i + 0].normal =   D3DXVECTOR3( sinf( theta )-1, 0.0f, cosf( theta ) );
    //    pVertices[2 * i + 1].position = D3DXVECTOR3( sinf( theta )-1, 1.0f, cosf( theta ) );
    //    pVertices[2 * i + 1].normal =   D3DXVECTOR3( sinf( theta )-1, 0.0f, cosf( theta ) );
    //}
    //g_pVB->Unlock();
    ...
    {
        FLOAT theta = (2 * D3DX_PI * i) / (50 - 1);
        pVertices2[2 * i + 0].position = D3DXVECTOR3(sinf(theta) + 1, -1.0f, cosf(theta));
        pVertices2[2 * i + 0].normal = D3DXVECTOR3(sinf(theta) + 1, 0.0f, cosf(theta));
        pVertices2[2 * i + 1].position = D3DXVECTOR3(sinf(theta) + 1, 1.0f, cosf(theta));
        pVertices2[2 * i + 1].normal = D3DXVECTOR3(sinf(theta) + 1, 0.0f, cosf(theta));
    }
    g_pVB2->Unlock();
...
VOID SetupLights()
{
    ...
    if (count++ >= 60)
    {
        ...
        {
        case 0:
            light.Diffuse.r += (rand() % 4) + 1;
            break;
        case 1:
            light.Diffuse.g += (rand() % 4) + 1;
            break;
        case 2:
            light.Diffuse.b += (rand() % 4) + 1;
            break;
        }
        count = 0;
    }
