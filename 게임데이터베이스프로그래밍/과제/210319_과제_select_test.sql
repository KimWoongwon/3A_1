...
select custid, sum(saleprice) as `김연아 구매액` from orders where custid = 2;
...
select orderdate, count(*) as `날짜별 판매건수` from orders group by orderdate having sum(saleprice) > 20000;
-- 22. 가장 구매건수가 많은 날짜를 조회 구매건수가 같은 경우 가장 최근 날짜를 조회
select orderdate, count(*) as 판매건수 from orders group by orderdate order by 판매건수 desc, orderdate desc limit 1;