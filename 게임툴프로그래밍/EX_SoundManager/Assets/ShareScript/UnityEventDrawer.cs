...
public class UnityEventDrawer : PropertyDrawer
{
    ...
    private void DrawEventListener(Rect rect, int index, bool isactive, bool isfocused)
    {
        ...
        else if (persistentListenerMode != PersistentListenerMode.Void && persistentListenerMode != PersistentListenerMode.EventDefined)
        {
            ...
            // AudioClipAttribute
            if (method != null)
                attributes = method.GetCustomAttributes(typeof(EX_AudioClipAttribute), true);
            if (attributes != null && attributes.Length > 0)
            {
                string tempval = "";
                AudioClip tempaudioclip = GetEX_AudioClipData(propertyRelative6.stringValue);
                var audioclipdata = EditorGUI.ObjectField(position3, tempval, tempaudioclip, typeof(AudioClip), true);
                if (audioclipdata != null)
                {
                    //propertyRelative6.objectReferenceValue = audioclipdata;
                    propertyRelative6.stringValue = audioclipdata.name;
                }
                else
                {
                    //propertyRelative6.objectReferenceValue = null;
                    propertyRelative6.stringValue = "";
                }
    ...
    AudioClip GetEX_AudioClipData(string p_url)
    {
...
//     void DrawEnumActionAttribute(EnumActionAttribute p_attribute )
//     {
// 
//     }
// 
//     void DrawEX_AudioClipAttribute(EX_AudioClipAttribute p_attribute)
//     {
// 
//     }
    ...
    {
        var typenemeserialized = prop.FindPropertyRelative("m_TypeName");
        if(typenemeserialized == null)
            return new UnityEvent();

        var type = Type.GetType(typenemeserialized.stringValue, false);
        if (type == null)
            return new UnityEvent();
